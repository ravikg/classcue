...
export async function getReminderSnapshot(context: HouseholdContext) {
  ...
  const [rulesResult, classJobsResult, feeJobsResult] = await Promise.all([
    ...
    d1.prepare(
      "SELECT jobs.id, rules.type, jobs.scheduled_for AS scheduledFor, jobs.status, jobs.sent_at AS sentAt, sessions.id AS sessionId, sessions.local_date AS localDate, sessions.planned_start_at AS plannedStartAt, sessions.timezone, COALESCE(sessions.location_override, enrollments.location) AS location, children.name AS childName, enrollments.display_name AS enrollmentName, contacts.name AS teacherName, contacts.phone AS teacherPhone FROM reminder_jobs jobs INNER JOIN reminder_rules rules ON rules.id = jobs.rule_id INNER JOIN sessions ON jobs.related_record_type = 'session' AND sessions.id = jobs.related_record_id INNER JOIN enrollments ON enrollments.id = sessions.enrollment_id INNER JOIN children ON children.id = enrollments.child_id LEFT JOIN enrollment_contacts links ON links.enrollment_id = enrollments.id AND links.role = 'teacher' AND links.is_primary = 1 LEFT JOIN contacts ON contacts.id = links.contact_id WHERE rules.household_id = ? AND jobs.status IN ('pending', 'delivered', 'dismissed') ORDER BY jobs.scheduled_for",
...
function classJob(row: ClassJobRow) {
  ...
  const body = `${row.childName}'s ${row.enrollmentName} is ${classTime}${row.location ? ` at ${row.location}` : ""}.${contact}`;
